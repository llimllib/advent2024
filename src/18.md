...
function search(map, start, end, debug = false) {
  ...
  while (frontier.length > 0) {
    ...
    for (const d of [N, E, S, W]) {
      ...
      if (newCost < (costSoFar.get(next) ?? Number.MAX_VALUE)) {
        ...
        // second
        frontier.unshift([newCost, next, d])
  ...
  if (debug) showdebug(map, cameFrom, costSoFar, start, end)
  return [costSoFar, cameFrom]
}

function pathlen(map, rocks, n, debug = false) {
  const m = clone(map)
  for (const [x, y] of rocks.slice(0, n)) {
    m[y][x] = WALL
  }
  let end = [m.length - 1, m[0].length - 1]
  let res = search(m, [0, 0], end, debug)
...
```js echo
function showdebug(map, cameFrom, costSoFar, start, end) {
  const width = 640
  const height = 640
